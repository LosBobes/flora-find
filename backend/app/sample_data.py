"""Sample FloraFind plants around Belgrade & Serbia, plus the helper that loads
them into a database.

This lives inside the ``app`` package (rather than only in the top-level
``seed.py`` script) so the FastAPI app can auto-seed an empty production
database at startup, and the CLI ``seed.py`` can reuse the exact same data.
"""
...
CONTRIBUTOR = {
    "email": "seed@florafind.rs",
    "username": "belgrade_forager",
    "password": "seed1234",
}
...
def ensure_seed_accounts(db: Session):
    """Create (or promote) the admin + contributor accounts. Returns the
    contributor, which owns the seeded plants."""
...
def seed_sample_plants(db: Session, *, force: bool = False) -> int:
    """Load the Belgrade/Serbia sample plants (and their owner accounts) into
    ``db``. Skips the plant insert when trees already exist unless ``force`` is
    set. Returns the number of plants inserted (0 if skipped).

    Idempotent and safe to call on every startup: it only inserts plants into an
    empty (or forced) database, and always tops up the plant-type vocabulary.
    """
    contributor = ensure_seed_accounts(db)

    existing = db.query(Tree).count()
    if existing and not force:
        backfill_plant_types(db)
        return 0
